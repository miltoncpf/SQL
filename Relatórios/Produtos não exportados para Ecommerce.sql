/*Lista os produtos não marcados para exportação para ecommerce em 
 * Cadatros > Produtos aba E-commerce / CISSFly 
         ...
         WHERE    UPPER(PRODUTOS_VIEW.FLAGINATIVO) = 'F' AND
			  ...
			  PRODUTOS_VIEW.IDSUBPRODUTO NOT IN
					(SELECT 
						IDSUBPRODUTO
		 			FROM 
						PRODUTO_GRADE_ECOMMERCE) AND
                  PRODUTO_GRADE_SALDO_EST_VW.IDEMPRESA = POLITICA_PRECO_PRODUTO.IDEMPRESA AND
                    PRODUTO_GRADE_SALDO_EST_VW.IDLOCALESTOQUE IN (1, 21, 31)
         ) AS TMP
